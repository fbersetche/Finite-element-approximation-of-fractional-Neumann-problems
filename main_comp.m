clc
s = 0.5;
%rad = rc(fix(length(rc)/4));

f = @(x,y) 2;%sin( pi*sqrt( x.^2 + y.^2 ) );%-(rad^2-1); %2*(1 - 1/R); 
g = @(x,y)  -1/( sqrt(x^2 + y^2) )^3;%double(sqrt(x^2 + y^2)<=rad+0.0001)  ;% -1/( sqrt(x^2 + y^2) )^3;
cns = s*2^(-1+2*s)*gamma(1+s)/(pi*gamma(1-s));
load('data.mat');
nn = size(p,2); % number of nodes

%%%%%
%complemento = setdiff(1:nn , nf);
%nf = 1:nn;
%%%%%
...
for l=1:nt-nt_aux % Main Loop
    ...
    JC = comp_quad(Bl,xl(1),yl(1),s,secphi,R,area(l),p_I,w_I,p_T_12);
    K(nodl, nn + 1) = K(nodl, nn + 1) + JC(:,1);
    K(nn + 1, nodl) = K(nn + 1, nodl) + ( JC(:,1) )';
    
    JC2 = comp_quad(Bl,xl(1),yl(1),s,secphi2,R,area(l),p_I,w_I,p_T_12);
...
for l=nt-nt_aux+1:nt % dato Neumann non hom 
    nodl = t(l,:);
    xl = p(1 , nodl); yl = p(2 , nodl);
...
M = zeros(nn+1,nn+1); %matriz de masa 
for l=1:nt-nt_aux
    nodl = t(l,:);
    M(nodl,nodl) = M(nodl,nodl) + (area(l)/12).*( ones(3) + eye(3) );
end

%%% ESTO DEBERIA IR, MIRAR BIEN!

%n_nol = setdiff(1:nn+1,nf);
%M(n_nol,n_nol) = 0;
...
alfa = 1;
K = K.*cns;
uh = (K + alfa.*M)\b; % Solving the linear system
%uh = (K)\b;

tita = 0:(2*pi)/100:2*pi;
xx = R.*cos(tita);
yy = R.*sin(tita);

zz = uh(nn+1).*ones(size(tita));
figure
hold on
plot3(xx, yy, zz , '-r')
trimesh(t(1:nt , :), p(1,:),p(2,:),uh(1:end-1)); % Graphic representation
hold off
%plot3(xx, yy, zz , '-r')
figure
uh2 = uh;
mi = min( min( uh(t(1:nt-nt_aux,:)) ) );
uh2( uh2<mi ) = mi;
trimesh(t(1:nt - nt_aux, :), p(1,:),p(2,:),uh2(1:end-1));
zlim([ mi , max(uh)])

%save('ejemplo_s01.mat', 'R', 'uh', 't', 'p', 'nt', 'nt_aux', 'nn' )

%autovalores 

%autoval = sort( eig(K\M) );

%sum( sum( K(complemento,complemento) ) )
%spy( K(complemento,complemento) )

%ureal = @(x,y)  1;

% for l=1:nn
%     u(l)=ureal(p(1,l),p(2,l));